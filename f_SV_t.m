function q_VAR = f_SV_t(thetahat,w_T,part_T,N)

%thetahat = [mu muh phih omega2_h nu]
...
yhat_T1 = thetahat(1)+trnd(thetahat(5)*ones(1,N)).*exp(part(2,:)/2);
q_VAR_T1 = quantile(yhat_T1,[0.01 0.025 0.05 0.99 0.975 0.95]);

resampidx = randsample(N,N,true,w_T); 
part(6,:) = part(6,resampidx); 
yhat_T5 = thetahat(1)+trnd(thetahat(5)*ones(1,N)).*exp(part(6,:)/2);
q_VAR_T5 = quantile(yhat_T5,[0.01 0.025 0.05 0.99 0.975 0.95]);

resampidx = randsample(N,N,true,w_T); 
part(21,:) = part(21,resampidx); 
yhat_T20 = thetahat(1)+trnd(thetahat(5)*ones(1,N)).*exp(part(21,:)/2);
