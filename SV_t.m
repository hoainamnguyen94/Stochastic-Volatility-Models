function thetahat = SV_t(y,T,nloop,burnin)
...
for loop = 1:nloop 
    ...
    %% sample lam
    lam = 1./gamrnd((nu+1)/2,2./(nu+(y-mu).^2./exph));
    
    %% sample nu                
    sum1 = sum(log(lam));
    sum2 = sum(1./lam);
    fnu = @(x) T*(x/2.*log(x/2)-gammaln(x/2)) - (x/2+1)*sum1 - x/2*sum2;
    dfnu = @(x) T/2*(log(x/2)+1-psi(x/2)) - .5*(sum1+sum2);  
    d2fnu = @(x) T/(2*x) - T/4*psi(1,x/2);                   
    err_nu = 1;
    nut = nu;
    while abs(err_nu) > 10^(-3) 
        err_nu = dfnu(nut);
        Knu = -d2fnu(nut);      
        nut = nut + Knu\err_nu;
        if nut<2
    ...
    if loop>burnin
        i = loop-burnin;        
        store_h(i,:) = h';         
        store_theta(i,:) =  [mu muh phih omegah2 nu];        
    end    
